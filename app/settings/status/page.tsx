...
export default function Statuses() {
  ...
  const { data: categorizedStatuses, isLoading: isStatusesLoading, error: statusesError } = useStatuses(true)
            ...
            >
              {Object.entries(categorizedStatuses ?? {}).map(([category, tags]) => (
                <div key={category} className="mt-6">
                  ...
                  <ul role="list" className="divide-y divide-gray-200 dark:divide-gray-800 mt-3 divide-dashed pl-8">
                      {tags.sort((a:any, b:any) => Number(a.id) - Number(b.id)).map((tag: Status) => (
                          <li key={tag.id} className="flex items-center justify-between py-2.5">
                            <Badge 
                              className="inline-flex items-center whitespace-nowrap rounded-md px-2 py-1 text-xs font-medium gap-2" 
                              showSquare 
                              variant={tag.style_variant as BadgeProps["variant"] ?? "default"}
                            >
                              {tag.label}
                            </Badge>
